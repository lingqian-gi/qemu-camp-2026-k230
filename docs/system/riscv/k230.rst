...
AXI bus and modelled as a ``SysBusDevice`` with ``memory_region_init_ram``.

Clock gating is controlled by the Clock Management Unit (CMU at
``0x91100000``, offset ``0x5c``) and reset by the Reset Management Unit (RMU at
``0x91101000``, offsets ``0x60``/``0x64``).  These system-controller peripherals
